...
public class Index implements Serializable, Closeable {
    ...
    public void test(Bank_Account acc) throws KeyNotUniqueException {
        assert (acc != null);
        //check requirements for unique keys
        if (isbns.contains(acc.strISBN)) {
            throw new KeyNotUniqueException(acc.strISBN);
        }
    }

    public void put(Bank_Account acc, long value) throws KeyNotUniqueException {
        test(acc);
        isbns.put(acc.strISBN, value);
        authors.put(acc.author, Bank_Account.authorDel, value);
        names.put(acc.name, value);
    }
